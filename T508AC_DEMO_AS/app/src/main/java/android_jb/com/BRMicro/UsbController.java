...
public class UsbController {
	...
	public UsbController(Activity parentActivity,
			IUsbConnState connectionHandler, int vid, int pid) {
	...
	private void enumerate(IPermissionListener listener) {
		...
		while (deviter.hasNext()) {
			...
			// DebugManage.WriteLog2("Found device: " +
			// String.format("%04X:%04X", d.getVendorId(), d.getProductId()));
			...
			if (d.getVendorId() == VID && d.getProductId() == PID) {
				...
				if (!mUsbManager.hasPermission(d)) {
					...
					// "enumerate, hasPermission return false" ,
					// Toast.LENGTH_SHORT).show();
					listener.onPermissionDenied(d);
				} else {
					// Toast.makeText(mApplicationContext,
					// "enumerate, GetConnInerface start" ,
					// Toast.LENGTH_SHORT).show();
					// startHandler(d);
					GetConnInerface(d);
					// TestComm(d);
		...
		if (bFound == false) {
			Toast.makeText(mApplicationContext, "no more devices found",
					Toast.LENGTH_SHORT).show();
	...
	private void GetConnInerface(UsbDevice dev) {
		...
		// Toast.makeText(mApplicationContext, "GetConnInerface start",
		// Toast.LENGTH_SHORT).show();
		...
		// Toast.makeText(mApplicationContext, "GetConnInerface start:in=" +
		// m_nEPInSize + ",out=" + m_nEPOutSize, Toast.LENGTH_SHORT).show();
		...
		// Toast.makeText(mApplicationContext,
		// "GetConnInerface OK, Out Max Size="+m_nEPOutSize+" In Max Size=" +
		// m_nEPInSize, Toast.LENGTH_SHORT).show();

		// DebugManage.WriteLog2("device connected");
		mConnectionHandler.onUsbConnected();
	}

	public boolean OperationInternal(byte[] pData, int nDataLen, int nTimeOut,
			boolean bRead) {
	...
	public boolean UsbSCSIWrite(byte[] pCDB, int nCDBLen, byte[] pData,
		...
		w_abyTmp[8] = (byte) (nDataLen & 0xFF); // low
		w_abyTmp[9] = (byte) ((nDataLen >> 8) & 0xFF);
		w_abyTmp[10] = (byte) ((nDataLen >> 16) & 0xFF);
		w_abyTmp[11] = (byte) ((nDataLen >> 24) & 0xFF); // high

		w_abyTmp[12] = 0x00; // cCBWFlags DEVICE <--- HOST
		w_abyTmp[13] = 0x00; // cCBWlun
		w_abyTmp[14] = 0x10; // cCBWCBLength

		System.arraycopy(pCDB, 0, w_abyTmp, 15, nCDBLen);

		w_bRet = UsbBulkSend(w_abyTmp, 31, nTimeOut);
		// Log.d(TAG, "UsbSCSIWrite UsbBulkSend 1 w_bRet = " + w_bRet);

		if (!w_bRet)
			return false;

		w_bRet = UsbBulkSend(pData, nDataLen, nTimeOut);
		// Log.d(TAG, "UsbSCSIWrite UsbBulkSend 2 w_bRet = " + w_bRet);
		...
		// receive csw
		w_bRet = UsbBulkReceive(w_abyCSW, 13, nTimeOut);
		// Log.d(TAG, "UsbSCSIWrite UsbBulkReceive 3 w_bRet = " + w_bRet);
		if (w_bRet) {
	...
	public boolean UsbSCSIRead(byte[] pCDB, int nCDBLen, byte[] pData,
		...
		w_abyTmp[12] = (byte) 0x80; // cCBWFlags DEVICE-->HOST
		w_abyTmp[13] = 0x00; // cCBWlun
		w_abyTmp[14] = 0x10; // cCBWCBLength
		...
		// Toast.makeText(mApplicationContext,
		// "UsbSCSIRead, UsbBulkReceive Time : " + w_nTime ,
		// Toast.LENGTH_SHORT).show();
	...
	public boolean UsbSCSIRead(byte[] pCDB, int nCDBLen, byte[] pData,
			int[] nDataLen, int nTimeOut, boolean must) {
		if (!this.IsInit()) {
			Log.d(TAG, "UsbSCSIRead :USB not init.");
			return false;
		}
		...
		w_abyTmp[8] = (byte) (nDataLen[0] & 0xFF); // low
		w_abyTmp[9] = (byte) ((nDataLen[0] >> 8) & 0xFF);
		w_abyTmp[10] = (byte) ((nDataLen[0] >> 16) & 0xFF);
		w_abyTmp[11] = (byte) ((nDataLen[0] >> 24) & 0xFF); // high
		...
		w_bRet = UsbBulkReceive(pData, nDataLen, nTimeOut, must);
		// byteArrToHexString(pData, " ");
		w_nTime = SystemClock.elapsedRealtime() - w_nTime;
	...
	private boolean UsbBulkSend(byte[] pBuf, int nLen, int nTimeOut) {
		...
		for (i = 0; i < n; i++) {
			System.arraycopy(pBuf, i * m_nEPOutSize, m_abyTransferBuf, 0,
					m_nEPOutSize);

			w_nRet = m_usbConn.bulkTransfer(m_epOUT, m_abyTransferBuf,
					m_nEPOutSize, nTimeOut);
		...
		if (r > 0) {
			...
			w_nRet = m_usbConn.bulkTransfer(m_epOUT, m_abyTransferBuf, r,
					nTimeOut);
			// Log.d(TAG, "UsbBulkSend bulkTransfer  w_nRet = " + w_nRet + ",r="
			// + r);
		...
		// Log.d(TAG, "UsbBulkSend bulkTransfer success ");
	...
	private boolean UsbBulkReceive(byte[] pBuf, int nLen, int nTimeOut) {
		...
		// Toast.makeText(mApplicationContext, "UsbBulkReceive, Buf Len = " +
		// pBuf.length, Toast.LENGTH_SHORT).show();

		for (i = 0; i < n; i++) {
			w_nRet = m_usbConn.bulkTransfer(m_epIN, m_abyTransferBuf,
					m_nEPInSize, nTimeOut);
			// Log.d(TAG, "UsbBulkReceive bulkTransfer 1 w_nRet = " + w_nRet +
			// ",m_nEPInSize=" + m_nEPInSize);

			if (w_nRet != m_nEPInSize) {
				Log.d(TAG, "UsbBulkReceive bulkTransfer err: w_nRet = "
						+ w_nRet + ",m_nEPInSize=" + m_nEPInSize);
				return false;
			}

			System.arraycopy(m_abyTransferBuf, 0, pBuf, i * m_nEPInSize,
					m_nEPInSize);
		}

		if (r > 0) {
			w_nRet = m_usbConn.bulkTransfer(m_epIN, m_abyTransferBuf, r,
					nTimeOut);
			// Log.d(TAG, "UsbBulkReceive bulkTransfer 2 w_nRet = " + w_nRet +
			// ",r=" + r);
			// byteArrToHexString(m_abyTransferBuf, " ");

			if (w_nRet != r) {
				Log.d(TAG, "UsbBulkReceive bulkTransfer err1: w_nRet = "
						+ w_nRet + ",r=" + r);
				return false;
			}
		...
		// Log.d(TAG, "UsbBulkReceive success");
	...
	private boolean UsbBulkReceive(byte[] pBuf, int[] nLen, int nTimeOut,
		...
		n = nLen[0] / m_nEPInSize;
		r = nLen[0] % m_nEPInSize;
		...
		for (i = 0; i < n; i++) {
			w_nRet = m_usbConn.bulkTransfer(m_epIN, m_abyTransferBuf,
					m_nEPInSize, nTimeOut);
			// Log.d(TAG, "UsbBulkReceive bulkTransfer 1.0 w_nRet = " + w_nRet +
			// ",m_nEPInSize=" + m_nEPInSize);

			System.arraycopy(m_abyTransferBuf, 0, pBuf, nLen[0], w_nRet);
			nLen[0] += w_nRet;
			if (w_nRet != m_nEPInSize) {
				if (must)
					return false;
				else {
					// Log.d(TAG, "UsbBulkReceive success 1 get len :" +
					// nLen[0]);
					// byteArrToHexString(pBuf, " ");
					return true;
				}
			}
		}
		...
		if (r > 0) {
			w_nRet = m_usbConn.bulkTransfer(m_epIN, m_abyTransferBuf, r,
					nTimeOut);
			// Log.d(TAG, "UsbBulkReceive bulkTransfer 2.0 w_nRet = " + w_nRet +
			// ",r=" + r);
			// byteArrToHexString(m_abyTransferBuf, " ");

			System.arraycopy(m_abyTransferBuf, 0, pBuf, nLen[0], r);
			nLen[0] += r;
			if (must) {
				if (w_nRet != r)
					return false;
		...
		// Log.d(TAG, "UsbBulkReceive success 2 get len :" + nLen[0]);
		// byteArrToHexString(pBuf, " ");
