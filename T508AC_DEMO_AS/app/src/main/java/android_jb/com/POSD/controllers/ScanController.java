...
import android_jb.com.POSD.Preference;
import android_jb.com.POSD.util.CodeType;
...
public class ScanController {
	...
	//kongzhi 
	private String SCAN_PWR_EN = "/proc/jbcommon/gpio_control/Scan_CTL";// 1开，0关
	...
	private static String serialPort_Path = "/dev/ttyS1"; // 串口地址
	...
	private int openserialPort(String port, int baudrate, int bits,
			char event, int stop) {
		try {
	...
	public int Barcode_Open(Context context, Callback dataReceive) {
		// ScanManager("/dev/ttySAC1", 9600, 8, 'N', 1);
		int ret = -1;
		serviceContext = context;
		this.onDataListener = dataReceive;
		power_up();
//		checkScannerModelSetting();
		setScannerModel(MODEL_EM3095);
		setScanPrefix();
		trig_scan = "0";
		trig_last = trig_scan;
...
//		Ioctl.activate(17, 0);
		if (!is_SerialPortOpen) {
			if (getScannerModel() == MODEL_N3680) {
				...
				ret = openserialPort(serialPort_Path, 115200, 8, 'N', 1);
			} else {
				ret = openserialPort(serialPort_Path, 9600, 8, 'N', 1);
			}
...
//						System.out.println("ScanActivity Barcode_Start");
	...
	public String checkScannerModelSetting() {
		...
		try {
			...
			//Log.e(TAG, "checkScannerModelSetting Error!");
			e.printStackTrace();
		}
		return text;
	}
			...
			if (!trig_last.equals(trig)) {
				try {
					if (trig_last.equals(trig_scan)) {
						Thread.sleep(120);
						 //Log.d("trig", "scan() sleep(120) " + "trig:" + trig);
					}
					doScan(trig);
				} catch (InterruptedException e) {
					e.printStackTrace();
	...
	private void doScan(String trig) {
		if (trig.equals("1")) {
			// keyF2DownOrNot = true;
			//System.out.println("stopReceice:"+stopReceice.hashCode());
			try {
			//Ioctl.activate(17, 1);
...
//				Log.e("trig", "scan:" + all_scan_count);
		...
		Log.d("trig", "trig:" + trig);
		 //Log.d("scan_time", "scan_time:" + scan_time);
		trig_last = trig;
					...
					try {
						...
						if (cout > 0) {
							long nowTime = System.currentTimeMillis();
							 Log.d("ReadThread_Handle_Start:", "" + nowTime);
							...
							if (onDataListener != null) {
								...
								System.out
										.println("ReadThread cutData_buffer: "
												+ Tools.bytesToHexString(cutData_buffer));
	...
	private void dealData(byte[] buffer, int size) {
		try {
			...
			if (buffer == null) {
			 ...
			 //Log.d(TAG, "buffer size:" + size + " buffer lenght:"
			// + buffer.length);
			...
			Log.v("ScanController","dealData Scan_mode: "+Scan_mode);
			switch (Scan_mode) {
			case Protocol_SE955_Protocol:
...
//						"SerialPortData_middle end:" + Tools.getNowTimeHm());
	...
	private void getNewLand_Em3070(byte[] buffer) {
		if (buffer.length >= 4 && buffer[0] == (byte) 0x11
			...
			//Log.i("time", "SerialPortData_total end:" + Tools.getNowTimeHm());
			...
			// 数据中间
			SerialPortData_middle(buffer);
			//Log.i("time", "SerialPortData_middle end:" + Tools.getNowTimeHm());
		} else if (buffer.length >= 2
				&& buffer[buffer.length - 1] == (byte) 0x14
				&& buffer[buffer.length - 2] == (byte) 0x12) {
			// 数据尾
	...
	private void SerialPortData_middle(byte[] portData) {
		if (null != serialPortData_buffer) {
			if (middle_index <= 5) {
				...
				if (getScannerModel() == MODEL_EM3095) {
					// 错误数据 ， 或者协议可能没写入
					 //Log.d(TAG, "SerialPortData_middle 错误数据 ， 或者3095协议可能没写入");
					serialPortData_buffer = null;
					middle_index = 0;
					initScanProtocol();
					init_index++;
